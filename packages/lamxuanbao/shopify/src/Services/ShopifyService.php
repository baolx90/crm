...
namespace LamXuanBao\Shopify\Services;
...
class ShopifyService implements ShopifyContract
{
    ...
    {
        $this->data = $shop;
        $this->setDomain($shop->myshopify_domain);
        $this->setToken($shop->access_token);
        return $this;
    }
    ...
    {
        $url = $this->domain.'/admin/api/'.$this->version.'/graphql.json';
        $client = new Client(
            $url,
            [
